...
| TUI | `ratatui` + `crossterm`. When the live config uses the OpenAI subscription integration, the footer fetches `/wham/usage` at startup and after completed turns and shows each reported rolling window’s percentage remaining and relative reset time; failed refreshes keep the last snapshot |
