...
2. **Transcript** — empty state is the Lua-logo moon (disk + gold satellite), not an astronaut. After the first **prompt** (not `/help`), splash goes away
   ...
   - Assistant: terminal-friendly CommonMark (gold headings, emphasis, inline/fenced code, links with visible destinations, lists, blockquotes, rules). Tables and images have no terminal layout; images render alt text plus URL
