...
| Missions | Linear append-only jsonl. Not a tree. Not Pi-compatible. The initial model is recorded when a mission is created. Each HTTP attempt records request routing and size metadata (`provider`, `model`, `api`, `url`, `input_items`, `input_bytes`, `attempt`) and each received HTTP response records `status` and `attempt`; no credentials, headers, or duplicated conversation content. Each model usage result is appended; reopening sums the mission totals for the footer and restores the latest prompt size |
